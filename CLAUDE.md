...
fix: ruff check --fix, ruff format → check: ruff check, ty check, `biston scan .`, `zorilla check tests`, `gerenuk run -- -q` (only the tests the diff can reach; full suite when non-Python files changed). Typical run well under 1 s. Timings in `.git/hook-timings.jsonl`, `uv run madoqua stats`.
